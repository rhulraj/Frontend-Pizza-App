...
import { getCartDetails, removeProductToCart } from "../../Redux/Slices/CartSlices";
...
function CartDetails() {
    ...
    async function handleRemove(productId) {
        ...
        const response = await dispatch(removeProductToCart(productId));
                  ...
                  <div className="space-y-4">
                            ...
                            cartDetails?.items.map((item) => {
                                return (
                                    <dd key={item?.product?._id} className="text-base font-medium ">
                                        {item?.product?.productName} x {item?.qauntity}

                                        <p>{item?.product?.price} x {item?.qauntity}</p>
                                    </dd>
                                )
                            })
                        }
                    ...
                    <dl className="flex items-center justify-between gap-4 pt-2 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <dd className="text-base font-bold ">
                        ...
                        {cartDetails?.items.length === 0
                          ? ''
                          : cartDetails?.items.reduce((acc, item) => acc + item?.qauntity*item?.product?.price , 0) }
                      </dd>
                    </dl>
                  </div>
                  {cartDetails?.items.length > 0 && (
